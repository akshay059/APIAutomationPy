...
'''locks need to be used along with cd because cd is causing parallel execution error.
but when using lock 1 and 2, somehow requests never complete.
there should not be any deadlock but something is wrong.

need to find alternative of cd and see if it solves the parallel execution issue.
'''
#import threading
#cdlock1 = threading.RLock()
#cdlock2 = threading.RLock()
...
class RestTest:
    ...
    def __init__(self):
        ...
        """
        self.HEADER_ENCODING ='ISO-8859-1' # Per RFC 2616
        global logging
        ...
        logging.basicConfig(format='%(levelname)s:%(message)s', filename='apitest.log', filemode='w')
        self.logger = logging.getLogger('pyresttest')

        # AUTOIMPORTS, these should run just before the main method, to ensure
        # everything else is loaded
        try:
            import jsonschema
            self.register_extensions('pyresttest.ext.validator_jsonschema')
    ...
    def read_test_file(self,path):
        ...
        # doc
        teststruct = yaml.safe_load(self.read_file(path))
        return teststruct
    ...
    def parse_headers(self,header_string):
        ...
        if sys.version_info < (2,7):
            header_msg = message_from_string(headers.encode(self.HEADER_ENCODING))
            return [(text_type(k.lower(), self.HEADER_ENCODING), text_type(v, self.HEADER_ENCODING))
    ...
    def parse_testsets(self,base_url, test_structure, test_files=set(), working_directory=None, vars=None):
                ...
                for key in node:
                    if key == u'import':
                        ...
                        if importfile not in test_files:
                            self.logger.debug("Importing test sets: " + importfile)
                            test_files.add(importfile)
                            import_test_structure = self.read_test_file(importfile)
                            with cd(os.path.dirname(os.path.realpath(importfile))):
                                import_testsets = self.parse_testsets(
                        ...
                        test_config = self.parse_configuration(
    ...
    def parse_configuration(self,node, base_config=None):
    ...
    def run_test(self,mytest, test_config=TestConfig(), context=None, curl_handle=None, *args, **kwargs):
        ...
        self.logger.debug("===================================")
        self.logger.debug("%s" % mytest.name)
        self.logger.debug("-----------------------------------")
        self.logger.debug("REQUEST:")
        self.logger.debug("%s %s" % (templated_test.method, templated_test.url))
        self.logger.debug("HEADERS:")
        self.logger.debug("%s" % (templated_test.headers))
        ...
        body.close()
        result.response_headers = text_type(headers.getvalue(), self.HEADER_ENCODING)  # Per RFC 2616
        headers.close()

        response_code = curl.getinfo(pycurl.RESPONSE_CODE)
        result.response_code = response_code

        self.logger.debug("Initial Test Result, based on expected response code: " +
        ...
        # Parse HTTP headers
        try:
            result.response_headers = self.parse_headers(result.response_headers)
        except Exception as e:
            trace = traceback.format_exc()
        ...
        if result.passed is True:
            ...
            if mytest.validators is not None and isinstance(mytest.validators, list):
                self.logger.debug("executing this many validators: " +
                ...
                self.logger.debug("no validators found")
    ...
    def run_benchmark(self,benchmark, test_config=TestConfig(), context=None, *args, **kwargs):
        ...
        self.logger.info('Warmup: ' + message + ' started')
        ...
        self.logger.info('Warmup: ' + message + ' finished')

        self.logger.info('Benchmark: ' + message + ' starting')
        ...
        self.logger.info('Benchmark: ' + message + ' ending')
    ...
    def analyze_benchmark_results(self,benchmark_result, benchmark):
    ...
    def metrics_to_tuples(self,raw_metrics):
    ...
    def write_benchmark_json(self,file_out, benchmark_result, benchmark, test_config=TestConfig()):
        """ Writes benchmark to file as json """
        json.dump(benchmark_result, file_out, default=safe_to_json)
    

    def write_benchmark_csv(self,file_out, benchmark_result, benchmark, test_config=TestConfig()):
    ...
    def log_failure(self,failure, context=None, test_config=TestConfig()):
        """ Log a failure from a test """
        self.logger.error("Test Failure, failure type: {0}, Reason: {1}".format(
            failure.failure_type, failure.message))
        if failure.details:
            self.logger.error("Validator/Error details:" + str(failure.details))
    ...
    def run_testsets(self,testsets):
        ...
        for testset in testsets:
            ...
            for test in mytests:
                ...
                result = self.run_test(test, test_config=myconfig, context=context, curl_handle=curl_handle)
                result.body = None  # Remove the body, save some memory!

                if not result.passed:  # Print failure, increase failure counts for that test group
                    # Use result test URL to allow for templating
                    self.logger.error('Test Failed: ' + test.name + " URL=" + result.test.url +
                                 " Group=" + test.group + " HTTP Status Code: " + str(result.response_code))

                    # Print test failure reasons
                    if result.failures:
                        for failure in result.failures:
                            self.log_failure(failure, context=context,
                    ...
                    self.logger.info('Test Succeeded: ' + test.name +
            ...
            for benchmark in mybenchmarks:  # Run benchmarks, analyze, write
                if not benchmark.metrics:
                    self.logger.debug('Skipping benchmark, no metrics to collect')
                    continue

                self.logger.info("Benchmark Starting: " + benchmark.name +
                            " Group: " + benchmark.group)
                benchmark_result = self.run_benchmark(
                    benchmark, myconfig, context=context)
                print(benchmark_result)
                self.logger.info("Benchmark Done: " + benchmark.name +
                            " Group: " + benchmark.group)
                ...
                if benchmark.output_file:  # Write file
                    ...
                    self.logger.debug("Benchmark writing to file: " +
    ...
    def register_extensions(self,modules):
        ...
        for ext in modules:
            ...
            extension_applies = {
                'VALIDATORS': self.validators.register_validator,
                'COMPARATORS': self.validators.register_comparator,
                'VALIDATOR_TESTS': self.validators.register_test,
                'EXTRACTORS': self.validators.register_extractor,
    ...
    def main(self, args):
        ...
        if 'log' in args and args['log'] is not None:
            self.logger.setLevel(self.LOGGING_LEVELS.get(
        ...
        test_file = args['test']
        test_structure = self.read_test_file(test_file)
        ...
        failures = self.run_testsets(tests)
        if __name__ == "__main__":
            sys.exit(failures)
        else :
            return failures
